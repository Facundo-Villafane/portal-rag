...
export function EmbedLayout({
    ...
    const sendMessage = useCallback(async (text: string) => {
        ...
        try {
            ...
            const assistantMsg = await response.text()
            const replyTime = new Date().toISOString()
            setMessages(prev => [...prev, { role: 'assistant', content: '', timestamp: replyTime }])

            setIsLoading(false)
            setIsRevealing(true)
            for (let i = TYPEWRITER_CHARS_PER_TICK; i <= assistantMsg.length + TYPEWRITER_CHARS_PER_TICK; i += TYPEWRITER_CHARS_PER_TICK) {
                const visibleText = assistantMsg.slice(0, i)
                setMessages(prev => {
                    const msgs = [...prev]
                    msgs[msgs.length - 1] = { role: 'assistant', content: visibleText, timestamp: replyTime }
                    return msgs
                })
                if (i < assistantMsg.length) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#fbf9ff]">
                        ...
                        <div key={i} className={cn('flex gap-2', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                            ...
                            {msg.role === 'user' ? (
                                <div className="max-w-[78%] flex flex-col items-end gap-0.5">
                                    <div className="rounded-2xl rounded-br-sm px-3.5 py-2.5 bg-blue-600 text-white shadow-lg shadow-blue-600/20">
                                ...
                                <div className="flex-1 min-w-0 group">
                                    <div className="rounded-2xl border border-[#dfe3f4] bg-white/90 px-4 py-3 text-slate-800 shadow-sm shadow-slate-200/60">
                                        <ReactMarkdown
            ...
            {/* Input */}
            <div className="flex-shrink-0 bg-[#fbf9ff] px-3 py-2.5">
                <form
                    onSubmit={e => { e.preventDefault(); sendMessage(input) }}
                    className="flex items-end gap-2 bg-white border border-[#e3e6f3] rounded-full px-4 py-2.5 focus-within:border-blue-200 transition-colors shadow-lg shadow-slate-200/70"
                >
                    <textarea
                        rows={1}
                        value={input}
